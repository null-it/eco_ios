...
class WashingInfoView: UIView {
    ...
    lazy var happyHoursInfoView: HappyHoursInfoView = .fromNib()!
...
//        if isSE {
//            constraintsSE.forEach { (constraint) in
...
//        }
    ...
    private func updateHappyTimesViewHiddenness() {
        happyTimesView.isHidden = isHappyTimesViewHidden
        happyTimesView.layoutIfNeeded()
        if #available(iOS 11.0, *) {
            layoutIfNeeded()
        }
        frame.size.height = stackView.frame.height + MapViewConstants.infoViewBottomInset
